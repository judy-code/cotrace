...
GitHub Pages，網址是 `https://<GitHub 帳號>.github.io/my-project/`（`my-project`
是 repo 名稱）。**只有前端會部署**，`server/` 後端不會、也不能靠 GitHub Pages 執行
...
- `vite.config.js` 的 `base` 只有 `command === 'build'` 時才會是 `/my-project/`，
  本機 `npm run dev` 還是 `/`。**如果之後把 repo 改名，這裡的路徑要一起改**
- `src/main.jsx` 用 `HashRouter` 而不是 `BrowserRouter`：GitHub Pages 是純靜態
  ...
  `.../my-project/#/explore/123`），瀏覽器永遠先拿到 `index.html` 再由前端解析路由，
  就不會有這個問題。代價是網址會多一個 `#`
